import Link from 'next/link'
import { MIMO_CONTACT, MIMO_SOCIAL, MIMO_COMPANY } from '@/lib/constants'
import { Button } from '@/components/ui/button'
import { getWhatsAppContactUrl } from '@/lib/constants'
import { APP_VERSION } from '@/lib/version'

/**
 * footer minimalista e humano.
 * 
 * - endereço completo + mapa integrado (Google Maps via API key)
 * - links essenciais (serviços, sobre, galeria, etc)
 * - redes sociais com ícones
 * - copyright dinâmico
 * 
 * @returns {JSX.Element} componente de footer
 */
export function Footer() {
  const footerLinks = [
    { href: '/servicos', label: 'Serviços' },
    { href: '/sobre', label: 'Sobre' },
    { href: '/galeria', label: 'Galeria' },
    { href: '/trabalhe-aqui', label: 'Trabalhe Aqui' },
    { href: '/blog', label: 'Blog' },
  ]

  return (
    <footer className="bg-mimo-blue text-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          {/* Endereço e Contato */}
          <div>
            <h3 className="font-bueno text-xl font-bold mb-4">Contato</h3>
            <address className="font-satoshi text-sm not-italic space-y-2">
              <p>{MIMO_CONTACT.address.street}</p>
              <p>
                {MIMO_CONTACT.address.neighborhood}, {MIMO_CONTACT.address.city} - {MIMO_CONTACT.address.state}
              </p>
              <p>CEP: {MIMO_CONTACT.address.zipCode}</p>
              <p className="mt-4">
                <a
                  href={`tel:${MIMO_CONTACT.phone}`}
                  className="hover:text-mimo-gold transition-colors"
                >
                  {MIMO_CONTACT.phone}
                </a>
              </p>
              <p>
                <a
                  href={`mailto:${MIMO_CONTACT.email}`}
                  className="hover:text-mimo-gold transition-colors"
                >
                  {MIMO_CONTACT.email}
                </a>
              </p>
            </address>
          </div>

          {/* Links */}
          <div>
            <h3 className="font-bueno text-xl font-bold mb-4">Links</h3>
            <nav className="space-y-2">
              {footerLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="block font-satoshi text-sm hover:text-mimo-gold transition-colors"
                >
                  {link.label}
                </Link>
              ))}
            </nav>
          </div>

          {/* Redes Sociais e Mapa */}
          <div>
            <h3 className="font-bueno text-xl font-bold mb-4">Redes Sociais</h3>
            <div className="flex space-x-4 mb-6">
              <a
                href={MIMO_SOCIAL.instagram}
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-mimo-gold transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.98-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.98-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href={MIMO_SOCIAL.facebook}
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-mimo-gold transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>

            <div className="mt-6">
              <Button
                variant="primary"
                href={getWhatsAppContactUrl()}
                external
                className="w-full bg-white text-mimo-brown hover:bg-mimo-neutral-light text-xl py-5"
              >
                Falar no WhatsApp
              </Button>
            </div>
          </div>
        </div>

        {/* Mapa */}
        {process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY && (
          <div className="mb-8 rounded-lg overflow-hidden">
            <iframe
              src={`https://www.google.com/maps/embed/v1/place?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}&q=${encodeURIComponent(MIMO_CONTACT.address.street + ', ' + MIMO_CONTACT.address.city)}`}
              width="100%"
              height="300"
              style={{ border: 0 }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              title="Localização Mimo"
            />
          </div>
        )}

        {/* Copyright */}
        <div className="border-t border-white/20 pt-8 text-center">
          <p className="font-satoshi text-xs text-white/60">
            © {new Date().getFullYear()} {MIMO_COMPANY.name}. Todos os direitos reservados.
          </p>
          <p className="font-satoshi text-xs text-white/40 mt-2">
            v{APP_VERSION}
          </p>
        </div>
      </div>
    </footer>
  )
}

